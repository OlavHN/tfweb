...
class JsonHandler(object):
    ...
    async def handler(self, request):
        try:
            ...
            if method not in self.batcher.batched_queues and method not in self.batcher.direct_methods:
            ...
            if not batch_result:
                return web.json_response({'error': 'Batch request failed'}, status=400)
